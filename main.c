...
TipoPokemon getTipo() {
    int tipo;
    printf("Escolha o tipo do Pokemon (0 - Fogo, 1 - Água, 2 - Grama, 3 - Elétrico, 4 - Normal, 5 - Psíquico, 6 - Inseto, 7 - Luta, 8 - Pedra, 9 - Fada, 10 - Fantasma, 11 - Voador): ");
    scanf("%d", &tipo);
...
void capturePokemon(Player* player) {
    ...
    Pokemon wildPokemon;
    sprintf(wildPokemon.name, "Pokemon Selvagem %d", rand() % 100);  
    wildPokemon.level = rand() % 10 + 1; 
...
void trainPokemon(Player* player) {
    ...
    pokemon->attack = pokemon->level * 2; 
    printf(GREEN "%s treinou! Novo nivel: %d, Saude: %d, Ataque: %d\n" RESET_COLOR, 
